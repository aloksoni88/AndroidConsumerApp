...
public class RangeBar extends View {
    ...
    private static final int DEFAULT_BAR_COLOR = 0x40000000;
    //private static final int DEFAULT_BAR_COLOR = 0x000000;

    private static final int DEFAULT_TEXT_COLOR = 0xf83618;

    private static final int DEFAULT_TICK_COLOR = 0xffffff;

    // Corresponds to material indigo 500.
    //private static final int DEFAULT_PIN_COLOR = 0xfffff;
    private static final int DEFAULT_PIN_COLOR = 0x000000;

    private static final float DEFAULT_CONNECTING_LINE_WEIGHT_PX = 4;

    // Corresponds to material indigo 500.
    private static final int DEFAULT_CONNECTING_LINE_COLOR = 0xfff83618;
    ...
    private static final float DEFAULT_BAR_PADDING_BOTTOM_DP = 14;
...
//        Log.i("ds", "Size "+rangeList.size());
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        final float marginLeft = Math.max(mExpandedPinRadius, mCircleSize) + padding;

        final float barLength = w - (2 * marginLeft);
        mBar = new Bar(ctx, marginLeft, yPos, barLength - padding, mTickCount, mTickHeightDP, mTickColor,
        ...
        if (mIsRangeBar) {
            ...
            if (rangeList != null) {
                mLeftThumb.setXValue(rangeList.get(mLeftIndex));
            } else
                mLeftThumb.setXValue(getPinValue(mLeftIndex));
        }
        mRightThumb.setX(marginLeft + (mRightIndex / (float) (mTickCount - 1)) * barLength - padding);
//        mRightThumb.setXValue(getPinValue(mRightIndex));
        if (rangeList != null) {
            mRightThumb.setXValue(rangeList.get(mRightIndex));
        } else {
            mRightThumb.setXValue(getPinValue(mRightIndex));
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int scaledSize = getResources().getDimensionPixelSize(R.dimen.budget_font_size);
        Paint mPaint = new Paint();
        mPaint.setTextSize(scaledSize);
        Typeface tf = Typeface.createFromAsset(getContext().getAssets(), "fonts/FiraSans-Medium.ttf");
        mPaint.setTypeface(tf);
        mPaint.setColor(Color.parseColor("#AA000000"));
        mPaint.setAntiAlias(true);
        canvas.drawText("BUDGET", 0, 50, mPaint);
...
//            mRightThumb.setXValue(getPinValue(mRightIndex));
            if (rangeList != null) {
                if (mRightIndex > (rangeList.size() - 1)) {
                    mRightIndex = rangeList.size() - 1;
                }
                mRightThumb.setXValue(rangeList.get(mRightIndex));
            } else {
                mRightThumb.setXValue(getPinValue(mRightIndex));
        ...
        if (mArePinsTemporary) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    //Log.i(TAG, "pressPin,onAnimationUpdate: -> "+ animation.getAnimatedFraction());
...
//        thumb.setXValue(getPinValue(tickIndex));
        ...
        if (rangeList != null) {
            try {
                thumb.setXValue(rangeList.get(tickIndex));
            } catch (IndexOutOfBoundsException ex) {
                thumb.setXValue(rangeList.get(rangeList.size() - 1));
            }
        } else
            thumb.setXValue(getPinValue(tickIndex));
        if (mArePinsTemporary) {
            ...
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    //Log.i(TAG, "releasePin,onAnimationUpdate: -> "+ animation.getAnimatedFraction());
                    mThumbRadiusDP = (Float) (animation.getAnimatedValue());
