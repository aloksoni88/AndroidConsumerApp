...
class PinView extends View {
    ...
    public void init(Context ctx, float y, float pinRadiusDP, int pinColor, int textColor,
        ...
        mPin = ContextCompat.getDrawable(ctx, R.drawable.rectangle_1);//rotate);
    ...
    public void draw(Canvas canvas) {
        ...
        strokeCircle.setColor(Color.parseColor("#AA000000"));
        strokeCircle.setStrokeWidth(5);
        canvas.drawCircle(mX, mY , mCircleRadiusPx, strokeCircle);
        //Draw pin if pressed
        if (mPinRadiusPx > 0 && (mHasBeenPressed || !mPinsAreTemporary)) {
                mBounds.set((int) mX - mPinRadiusPx - 20,
                        (int) mY - (mPinRadiusPx * 2) - (int) mPinPadding, //-padding
                        (int) mX + mPinRadiusPx + 20, (int) mY - (int) mPinPadding);
            ...
            canvas.drawText(text,
                    mX,
                    mY - mPinRadiusPx - mPinPadding + mTextYPadding - 5,//-5 addede
                    mTextPaint);
        }
    }
    ...
    private void calibrateTextSize(Paint paint, String text, float boxWidth) {
        ...
        paint.setTextSize(10*mDensity);//estimatedFontSize * mDensity);
    }
}
